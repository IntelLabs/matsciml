...
class ForceRegressionTask(BaseTaskModule):
    ...
    def process_embedding(
        self, embeddings: Embeddings, pos: torch.Tensor, fa_rot: None | torch.Tensor = None,
    ) -> dict[str, torch.Tensor]:
        outputs = {}
...
class GradFreeForceRegressionTask(ScalarRegressionTask):
    ...
    def process_embedding(self, embeddings: Embeddings, fa_rot: None | torch.Tensor = None) -> dict[str, torch.Tensor]:
        """
...
class MultiTaskLitModule(pl.LightningModule):
    ...
    def _toggle_input_grads(
        self,
        batch: dict[
            str, dict[str, torch.Tensor | dgl.DGLGraph | dict[str, torch.Tensor]],
        ],
    ) -> None:
        """
...
class IS2REInference(OpenCatalystInference):
    def __init__(
        self, pretrained_model: AbstractEnergyModel | ScalarRegressionTask,
    ) -> None:
        assert isinstance(
