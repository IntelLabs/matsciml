...
from .geometric_algebra import vector_vector, vector_vector_invariants
...
class LayerNorm(torch.nn.Module):
    def forward(self, x):
        ...
        sigma = torch.sqrt(torch.maximum(sigmasq,(torch.ones(sigmasq.shape)*torch.as_tensor(1e-6)).to(sigmasq.device)))
...
class VectorAttention(nn.Module):
    def __init__(
        ...
        self.value_net = ValueNet(
            self.invariant_dims, hidden_feats, in_feats, dropout=dropout)
    ...
    def _calculate_attention(
        ...
        attention = torch.softmax(
            scores.flatten(scores.dim() - 3), dim=-1).view(scores.shape)
