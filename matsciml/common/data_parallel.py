...
class ParallelCollater_DGL:
    ...
    def __call__(self, data_list):
        if self.num_gpus in [0, 1]:  # adds cpu-only case
            batch, labels = data_list_collater_dgl(data_list, otf_graph=self.otf_graph)
            ...
            return [
                data_list_collater_dgl(data_list[split[i] : split[i + 1]])
                for i in range(len(split) - 1)
            ]


class ParallelCollater_GAANet:
    def __init__(self, num_gpus, *args, **kwargs):
        self.num_gpus = num_gpus
