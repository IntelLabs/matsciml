...
parser.add_argument("--data-path", type=Path, default="../data", help="Path to move the unpacked data to.")
...
for index, entry in enumerate(record["files"]):
    ...
    if record_checksum == repo_checksum:
        ...
        break
if not valid:
    raise ValueError(f"No match for repository checksum on Zenodo servers. Expected: {repo_checksum}")
...
assert dl_checksum == record_checksum, f"Checksum mismatch; expected: {checksum}, downloaded: {dl_checksum}. Contact maintainers!"
