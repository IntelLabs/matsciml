...
from ocpmodels.models.base import BaseTaskModule

from ocpmodels.models.diffusion_utils import MAX_ATOMIC_NUM, KHOT_EMBEDDINGS
from ocpmodels.models.diffusion_utils.data_utils import (
    EPSILON, cart_to_frac_coords, frac_to_cart_coords, mard,
    lengths_angles_to_volume, min_distance_sqr_pbc
)
...
class GenerationTask(BaseTaskModule):

    __task__ = "generation"

    """
    Generative modeling task. Right now it implements CDVAE
    but consider abstracting the diffusion pipeline to decouple
    scheduling/sampling from the denoising model.
    ...
    def forward(
        ...
        # VAE stuff
        mu, log_var, z = self.encode(embedding)
        # Decode Z
    ...
    def training_step(
        ...
        loss_dict = self._compute_losses(batch, teacher_forcing)
    ...
    def _make_output_heads(self) -> nn.ModuleDict:
        modules = {}
        # for CDVAE we need several heads for 7 loss terms
        # 1) num_atoms predictor for num_atom_loss
        # 2) lattice predictor for lattice_loss
        # 3) composition predictor for composition_loss
        # 4) optional property predictor for property_loss
        # 5) coordinate loss
        # 6) type loss
        # 7) KL div loss
        # and mu/var MLPs for VAE reparameterization
