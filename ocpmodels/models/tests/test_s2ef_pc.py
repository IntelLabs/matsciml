...
going from model instantiation to S2EFPointCloudModule instantiation,
...
devset_module = PointCloudDataModule.from_s2ef(
    train_path=s2ef_devset, batch_size=8, num_workers=0
)
devset_module.setup()


class FakePointCloudModule(nn.Module):
    def __init__(self):
        super().__init__()
        self.feat_linear = nn.Linear(1, 16)
        self.pos_linear = nn.Linear(3, 16)
        self.output = nn.Sequential(nn.Linear(32, 1))

    def forward(self, features: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        transformed = torch.cat(
            [self.feat_linear(features), self.pos_linear(positions)], dim=-1
        )
        return self.output(transformed)


@pytest.fixture(scope="module")
def test_fakegaa_instantiation() -> nn.Module:
    ...
    fake_gaa = FakePointCloudModule()
    return fake_gaa


@pytest.fixture(scope="module")
def test_s2ef_instantiation(test_fakegaa_instantiation):
    """Test if an `S2EFPointCloudModule` object can be successfully created"""
    model = base.S2EFPointCloudModule(test_fakegaa_instantiation, lr=1e-3, gamma=0.1)
    return model
