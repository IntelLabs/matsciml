...
class GalaPotential(AbstractEnergyModel):
    ...
    """
    def __init__(self, D_in, hidden_dim=32, depth=2, dilation=2., residual=True,
                 ...
                 score_normalization=None, block_normalization=None, pc_size=16, pc_mini_batch=50):
        super().__init__()
        ...
        if self.nonlinearities:
            self.nonlin_mlps = torch.nn.ModuleList([
                self.make_value_net(self.hidden_dim, within_network=False) for _ in range(self.depth + 1)])

        self.block_norm_layers = torch.nn.ModuleList([])
        for _ in range(self.depth + 1):
            self.block_norm_layers.extend(self._get_normalization_layers(self.block_normalization, self.hidden_dim))
    ...
    def make_attention_nets(self):
        ...
        for i in range(self.depth + 1):
            ...
            self.eqvar_att_nets.append(gala.Multivector2MultivectorAttention(
                self.hidden_dim, self.score_nets[-1], self.value_nets[-1], self.scale_nets[-1],
            ...
            self.invar_att_nets.append(gala.MultivectorAttention(
                self.hidden_dim, self.score_nets[-1], self.value_nets[-1],
